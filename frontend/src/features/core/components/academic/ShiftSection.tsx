...
import { shiftSchema } from '~/features/core/schemas/academicSchemas';
import type { Shift } from '~/features/core/types/academic';
...
const ShiftSection = memo(() => {
  ...
  const [itemToDelete, setItemToDelete] = useState<Shift | null>(null);
   ...
   * Handle edit item
   */
  const handleEdit = useCallback((item: Shift) => {
   ...
   * Handle delete item
   */
  const handleDelete = useCallback((item: Shift) => {
  ...
  const onSubmit = async (data: FormData) => {
    try {
      if (isEdit && selectedItem) {
        ...
        await dispatch(updateShift({ id: selectedItem.id, data: changedFields })).unwrap();
        showToast(`Shift ${SUCCESS_MESSAGES.UPDATED}`, 'success');
      } else {
        await dispatch(createShift(data)).unwrap();
        showToast(`Shift ${SUCCESS_MESSAGES.CREATED}`, 'success');
      }
      handleFormModalClose();
    } catch (error: any) {
      showToast(error.message || `Failed to ${isEdit ? 'update' : 'create'} shift`, 'error');
    }
  };
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    try {
      await dispatch(deleteShift(itemToDelete.id)).unwrap();
      showToast(`Shift ${SUCCESS_MESSAGES.DELETED}`, 'success');
    ...
    <Box>
      ...
      {shifts.length > 0 ? (
        ...
        <EmptyState
          ...
          description="No shifts available. Add some to get started."
      ...
      <Dialog open={formModalOpen} onClose={handleFormModalClose} maxWidth="sm" fullWidth disableRestoreFocus>
        <DialogTitle>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Typography variant="h6" fontWeight={600}>
              {selectedItem ? 'Edit Shift' : 'Add Shift'}
            </Typography>
