...
import {
  createStaff,
  updateStaff,
  fetchAssignableStaffPersons,
  selectAssignableStaffPersons,
  selectAssignableStaffPersonsLoading,
  selectStaffLoading,
} from '~/features/education/staff/store/staffSlice';

import { staffSchema, type StaffFormData } from '~/features/education/staff/schemas/staffSchema';
...
const TabPanel = ({ children, value, index, ...other }: TabPanelProps) => (
  <div role="tabpanel" hidden={value !== index} id={`staff-form-tabpanel-${index}`} aria-labelledby={`staff-form-tab-${index}`} {...other}>
...
const StaffFormModal = memo(({ open, onClose, staff }: StaffFormModalProps) => {
  ...
  const assignablePersons = useAppSelector(selectAssignableStaffPersons);
  const assignableLoading = useAppSelector(selectAssignableStaffPersonsLoading);
  ...
  const includePersonId = staff?.person?.id;
  useEffect(() => {
    if (!open) return;
    dispatch(fetchAssignableStaffPersons({ page: 1, limit: 1000, includePersonId }));
  }, [dispatch, open, includePersonId]);

  // map StaffDetail -> StaffFormData (for edit)
  useEffect(() => {
    if (!open) return;

    if (staff) {
      const mapped: StaffFormData = {
        personStaffId: staff.person?.id ?? '',
        educationalQualifications:
          staff.educationalQualifications?.map(eq => ({
        ...
        professionalExperience:
          staff.professionalExperience?.map(pe => ({
        ...
        salaryExpectation: staff.salaryExpectation ?? undefined,
        joiningDate: (staff.joiningDate as any) ?? '',
        digitalSignatureUrl: (staff as any).digitalSignatureUrl ?? '',
        yearsOfExperience: staff.yearsOfExperience ?? undefined,
        noticePeriod: staff.noticePeriod ?? '',
        subjectIds: staff.subjectIds ?? [],
        gradeLevelIds: staff.gradeLevelIds ?? [],
        languageProficiencyIds: staff.languageProficiencyIds ?? [],
        computerSkills: staff.computerSkills ?? '',
        teachingMethodology: staff.teachingMethodology ?? '',
        onlineProfiles: {
          linkedin: staff.onlineProfiles?.linkedin ?? '',
          personalWebsite: staff.onlineProfiles?.personalWebsite ?? '',
        },
        details: staff.details ?? '',
        status: (staff.status as any) ?? STATUSES_OBJECT.INACTIVE,
      };
      reset(mapped);
    } else {
      reset(DEFAULTS);
    ...
    try {
      if (staff) {
        ...
        const base = {
          ...
          details: staff.details ?? '',
          status: (staff.status as any) ?? STATUSES_OBJECT.INACTIVE,
        } as unknown as StaffFormData;

        let diff = getChangedFields<StaffFormData>(base, data);

        // Avoid TS2322: arrays on Staff update expect items with id
        ...
        delete (diff as any).personStaffId;
        ...
        await dispatch(updateStaff({ id: staff.id, data: diff as any })).unwrap();
        showToast(`Staff ${SUCCESS_MESSAGES.UPDATED}`, 'success');
      } else {
    ...
    <Dialog open={open} onClose={onClose} maxWidth="lg" fullWidth>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
                ...
                <Grid size={{ xs: 12, md: 6 }}>
                  <FormSection title="Assign Staff Person" required>
                    <Controller
                      ...
                      control={control}
                      render={({ field }) => (
                        <FormControl fullWidth error={!!errors.personStaffId}>
                          <InputLabel>Staff Person *</InputLabel>
                          <Select
                            {...field}
                            label="Staff Person *"
                            disabled={!!staff /* don't reassign on edit */}
                            renderValue={(val) => {
                          ...
                          {errors.personStaffId && (
                            <Typography variant="caption" color="error" sx={{ mt: 0.5, ml: 2 }}>
                              {errors.personStaffId.message}
                            </Typography>
                          )}
                        </FormControl>
                      )}
        ...
        <DialogActions sx={{ p: 2 }}>
          ...
          >
            {loading || isSubmitting ? 'Saving...' : (staff ? 'Save' : 'Add')}
          </Button>
        </DialogActions>
      </form>
    </Dialog>
