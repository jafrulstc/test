// features/education/staff/components/RegisterForm.tsx
...
const TabPanel = ({ children, value, index, ...other }: TabPanelProps) => (
  <div role="tabpanel" hidden={value !== index} id={`staff-tabpanel-${index}`} aria-labelledby={`staff-tab-${index}`} {...other}>
...
const StaffRegisterForm = memo(({ onSuccess, onError }: RegisterFormProps) => {
  ...
  // staff slice states
  const loading = useAppSelector(selectStaffLoading); // Form submission loading
  ...
  const assignableLoading = useAppSelector(selectAssignableStaffPersonsLoading); // Data fetching loading
  ...
  useEffect(() => {
    ...
    dispatch(fetchAssignableStaffPersons({ page: 1, limit: 1000 })); // STAFF persons not yet assigned as staff
  }, [dispatch]);
  ...
  const onSubmit = async (data: StaffFormData) => {
    try {
      ...
      navigate('/admin/education/staff/manage');
      onSuccess?.();
    } catch (e: any) {
      const errorMessage = e?.message || 'Failed to create staff';
...
StaffRegisterForm.displayName = 'StaffRegisterForm';
