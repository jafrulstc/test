...
import { PersonFormValuesIn, PersonFormValuesOut, personSchema, type PersonFormData } from '~/features/education/person/schemas/personSchema';
...
const PersonForm = memo(({ person, onSuccess }: PersonFormProps) => {
    ...
    useEffect(() => {
        if (person) {
            const defaultValues: PersonFormValuesIn = {
                ...
                emergencyCont: person.emergencyCont ?? null,
    ...
    /** Submit */
    const onSubmit = async (raw: PersonFormValuesIn) => {
        try {
            const data: PersonFormValuesOut = personSchema.parse(raw); // status default apply, numbers coerce
            if (isStaffSelected) {
                if (!data.designationCategoryId) { showToast('Designation category is required for Staff', 'error'); return; }
                if (!data.designationId) { showToast('Designation is required for Staff', 'error'); return; }
            }
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            <TextField
                                ...
                                value={field.value ?? ''} // null হলে UI-তে খালি দেখাও
                                onChange={(e) => field.onChange(e.target.value === '' ? null : Number(e.target.value))}
